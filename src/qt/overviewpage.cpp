...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::Aegeus)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("Aegeus")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeAegeusAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAegeusAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeAegeusAmount = strAnonymizeAegeusAmount.remove(strAnonymizeAegeusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
    ...
    if (nMaxToAnonymize > nAnonymizeAegeusAmount * COIN) nMaxToAnonymize = nAnonymizeAegeusAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeAegeusAmount * COIN) {
